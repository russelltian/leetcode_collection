...
bool isSubtree(TreeNode* s, TreeNode* t) {
    //idea: start from a single node of s and recursively compare, then recursively move to the next ndoe of s, 94.25%
    if(!s)return false;
    if(isSubtree_helper(s,t))return true;
    return isSubtree(s->left,t)||isSubtree(s->right,t);
}
bool isSubtree_helper(TreeNode* s,  TreeNode* t){
    if(!s &&!t)return true;
    if(!s||!t)return false;
    if(s->val != t->val)return false;
    return  isSubtree_helper(s->left,t->left) && isSubtree_helper(s->right,t->right);//comparing
}
...
TreeNode* sortedArrayToBST(vector<int>& nums) {
	...
	int l = 0; int r = int(nums.size()) - 1;
	return sortedArrayToBST(nums, l, r);

}
TreeNode* sortedArrayToBST(vector<int>& nums, int l, int r) {
	//more thinking why l > r is termination condition
	int m = (l + r) / 2;
	if (l > r) {
		return NULL;
	}
